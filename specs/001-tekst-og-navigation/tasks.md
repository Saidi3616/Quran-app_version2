...
- `[P]` betyder, at opgaven *kan* laves parallelt med den foregående. `H1`–`H6` henviser til brugerhistorierne i spec'en.
...
## Milepæl 0: Fundament
*Resultat: Et tomt React-projekt, der kører, og hvor testene er sat op.*

- [ ] **T001 · Opret projektet**
  Opret et Vite-projekt med React og TypeScript. Tilføj `.gitignore` og scripts: `dev`, `build`, `test`, `lint`.
  *Du lærer:* npm, `package.json`, og hvad en udviklingsserver er.
  **Færdig når:** `npm run dev` viser en side i browseren, og `npm run build` virker uden fejl.

- [ ] **T002 · Kodekvalitet**
  Sæt ESLint og Prettier op.
  **Færdig når:** `npm run lint` kører uden fejl.

- [ ] **T003 · Testværktøjer**
  Sæt Vitest + Testing Library og Playwright op. Skriv én "hello world"-test af hver slags.
  *Du lærer:* hvad en test er, og hvorfor vi skriver dem.
  **Færdig når:** `npm test` og `npm run test:e2e` er grønne.

## Milepæl 1: Data på plads
*Resultat: Koranens tekst ligger i projektet, og testene beviser, at den er korrekt.*

- [ ] **T004 · Hent Tanzil-filerne** *(din opgave, se "Sådan henter du filerne" nederst)*
...
- [ ] **T005 · Undersøg filernes format**
  ...
  *Du lærer:* at læse XML og at undersøge data, før man skriver kode.
...
- [ ] **T008 · Test af dataintegritet** *(SC-003, SC-004)*
  Test at:
  ...
  **Færdig når:** testene er grønne. *Det er den vigtigste test i hele projektet (princip 1).*
...
*Resultat: Du kan åbne appen og læse Koranen.* 🎉
...
- [ ] **T013 · Komponenterne `Verse` og `VerseMarker`** *(FR-003, FR-004)*
...
- [ ] **T018 · Modulet `storage/`** *(FR-016)*
  ...
  *Du lærer:* at data fra "omverdenen" altid skal kontrolleres, før man bruger dem.
  **Færdig når:** enhedstestene er grønne, også testen hvor der ligger ødelagte data.

- [ ] **T019 · Oversættelse til/fra** *(FR-007, FR-008)*
...
*Network access* i miljøets indstillinger. Så kan Claude hente filerne og dokumentere præcis,
...
Upload dem til mappen `data/tanzil/` på GitHub via *Add file → Upload files* på branchen
